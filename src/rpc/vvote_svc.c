...
char *argv[];
{
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_DGRAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_UDP;
		if (!svc_register(transp, VOTINGSYS, VOTINGSYS_V1, votingsys_1, proto)) {
			_msgout("unable to register (VOTINGSYS, VOTINGSYS_V1, udp).");
			exit(1);
		}
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_STREAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_TCP;
		if (!svc_register(transp, VOTINGSYS, VOTINGSYS_V1, votingsys_1, proto)) {
			_msgout("unable to register (VOTINGSYS, VOTINGSYS_V1, tcp).");
			exit(1);
		}
	}
